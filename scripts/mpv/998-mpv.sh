...
./bootstrap.py
LDFLAGS='-Wl,-Bstatic -lpthread' python3 waf configure --prefix=$(realpath -m ${SCRIPT_ROOT}/../dist/${ARCH}_mpv${BUILD_EXTRA_SUFFIX}) -o ${BUILD_ROOT}/mpv-${MPV_VERSION} --enable-static-build --disable-manpage-build --enable-dvdread --enable-dvdnav --enable-libarchive
