...
patch_source mpv-${MPV_VERSION} 998-mpv.patch
sed -i -e "/^${fnprefix} /d" "${FINISHED}"

export PKG_CONFIG_PATH=$(realpath -m ${DIST_ROOT}/${ARCH}_mpv${BUILD_EXTRA_SUFFIX}/lib/pkgconfig):${PKG_CONFIG_PATH}
pushd ${SRC_ROOT}/mpv-${MPV_VERSION}
./bootstrap.py
rm -rf ${BUILD_ROOT}/mpv-${MPV_VERSION}
/usr/bin/python2 waf configure --prefix=$(realpath -m ${DIST_ROOT}/${ARCH}_mpv${BUILD_EXTRA_SUFFIX}) -o ${BUILD_ROOT}/mpv-${MPV_VERSION} --enable-static-build --disable-manpage-build --enable-dvdread --enable-dvdnav --enable-libarchive
